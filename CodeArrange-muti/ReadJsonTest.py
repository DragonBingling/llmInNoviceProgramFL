...
def extract_json_analyzer(string):
    # 使用正则表达式查找包含整个 JSON 数据的部分
    pattern = r'(?s)\{.*"analyze".*}'  # 匹配整个 JSON 数据，包括换行符和空格
    matches = re.findall(pattern, string, re.S)
    # xi
    for json_str in matches:
        try:
...
def extract_json_sorter(string):
    # 使用正则表达式查找包含整个 JSON 数据的部分
    pattern = r'(?s)\{.*"faultLocalization".*\[.*].*}'  # 匹配整个 JSON 数据，包括换行符和空格
    matches = re.findall(pattern, string, re.S)
    # xi
    for json_str in matches:
        try:
