...
def send_single_code_simulateRunner(mutiAgentPrompt, ans_path, code_index, code_info, experiment_model):
    global tokensum
    response_topN_location = os.path.join(ans_path, "topN.txt")
    problemAnalyzer_ans_location = os.path.join(ans_path, "problemAnalyzerAns.txt")
    simulateRunner_ans_location = os.path.join(ans_path, "simulateRunnerAns.txt")
    ...
    simulateRunnerPrompt = (mutiAgentPrompt['simulateRunner']
                           + "\n\n problemDescription:\n" + code_info['problemDescription']
                           + '\n\n input： ' + code_info['input']
                           + '\n\n output ' + code_info['output']
                           + '\n\n reference problem-solving ideas: \n' + problemAnalyzerAns
    ...
    # 判断token是否超出限制
    tokens = getTokenNumbers.get_openai_token_len(simulateRunnerPrompt, model="text-davinci-001")
    ...
    while repeat_time_this > 0:
        ...
        try:
            # 发送请求在这
            print(" simulateRunner，第 " + str(repeat_time - repeat_time_this) + " 次请求。" + str(code_index))
            response_txt = SendPromt.send_prompt_openai_gpt(simulateRunnerPrompt, experiment_model)
        except:
            print("simulateRunner 请求发送异常" + str(code_index))
            continue

        try:
            res_json_data = ReadJsonTest.extract_json_analyzer(response_txt)
        except:
            print("simulateRunner Json读取异常" + str(code_index))
            continue

        if res_json_data is None:
            print(str(code_index) + "simulateRunner json读取到空")
            continue
        else:

            with open(simulateRunner_ans_location, 'w', encoding='utf-8') as file:
                file.write(res_json_data['analyze'])

                # 跳出循环
            print("simulateRunner 数据存储成功 " + str(code_index))
            return True
            break
    return False
...
def send_single_code_faultSeeker_albation(mutiAgentPrompt, ans_path, code_index, code_info, experiment_model,experiment_index):
    global tokensum
    data_path = "D:/私人资料2024/论文/大模型相关/多代理新手程序错误定位实验/data/tutorcode/version/1/deepseek-r1/muti_simulateRunner_1"
    response_topN_location = os.path.join(ans_path, "topN.txt")
    problemAnalyzer_ans_location = os.path.join(data_path, "problemAnalyzerAns.txt")
    codeAnalyzer_ans_location = os.path.join(data_path, "codeAnalyzerAns.txt")
    faultSeeker_ans_location = os.path.join(ans_path, "faultSeekerAns.txt")
    simulateRunner_ans_location = os.path.join(data_path, "simulateRunnerAns.txt")
    ...
    with open(simulateRunner_ans_location, 'r', encoding='utf-8') as file:
        # 读取文件内容并保存到字符串中
        simulateRunnerAns = file.read()

    if experiment_index == "muti_no_ProblemAnalyze1":
        faultSeekerPrompt = (mutiAgentPrompt['faultSeeker']
                             + '\n\n reference ideas behind this incorrect code:\n' + codeAnalyzerAns
                             + '\n\n simulate test cases running:\n' + simulateRunnerAns
                             + '\n\n incorrect_code:\n' + code_info['incorrect_code_indexed']
                             )
    elif experiment_index == "muti_no_CodeAnalyze":
        faultSeekerPrompt = (mutiAgentPrompt['faultSeeker']
                             + '\n\n reference problem-solving ideas: \n' + problemAnalyzerAns
                             + '\n\n simulate test cases running:\n' + simulateRunnerAns
                             + '\n\n incorrect_code:\n' + code_info['incorrect_code_indexed']
                             )
    elif experiment_index == "muti_no_SimulateRunner":
        faultSeekerPrompt = (mutiAgentPrompt['faultSeeker']
                             + '\n\n reference problem-solving ideas: \n' + problemAnalyzerAns
                             + '\n\n reference ideas behind this incorrect code:\n' + codeAnalyzerAns
                             + '\n\n incorrect_code:\n' + code_info['incorrect_code_indexed']
...
def run_tutorcode_muti_albation(mutiAgentPrompt, experiment_index, experiment_model, rangeIndex):
    global tokensum
    ...
    for index, item in enumerate(tutor_data):
        ...
        # if not isok:
        #     print("codeAnalyzer发生异常，建议重试")
        #     return False
        #
        # isok = send_single_code_simulateRunner(mutiAgentPrompt, ans_path, index, item, experiment_model)
        # if not isok:
        #     print("simulateRunner发生异常，建议重试")
        #     return False

        isok = send_single_code_faultSeeker_albation(mutiAgentPrompt, ans_path, index, item, experiment_model,experiment_index)
        if not isok:
...
if __name__ == "__main__":
    ...
    # repeat = 10
    # while repeat>0:
    #     isok = run_tutorcode_muti_albation(mutiAgentPrompt, "muti_no_ProblemAnalyze1", "deepseek-r1", 200)
    #     print("结束一轮，休眠1s")
    #     time.sleep(1)
    #     repeat-=1
    ...
    # repeat = 10
    # while repeat > 0:
    #     isok = run_tutorcode_muti_albation(mutiAgentPrompt, "muti_no_CodeAnalyze", "deepseek-r1", 200)
    #     print("结束一轮，休眠60s")
    #     time.sleep(60)
    #     repeat -= 1
    ...
    repeat = 10
    while repeat > 0:
        isok = run_tutorcode_muti_albation(mutiAgentPrompt, "muti_no_SimulateRunner", "deepseek-r1", 200)
        print("结束一轮，休眠60s")
        time.sleep(60)
        repeat -= 1
