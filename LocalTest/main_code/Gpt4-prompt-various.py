...
if __name__ == "__main__":
    experiment_model = "gpt-4"
    base_location = "D:/私人资料/论文/大模型相关/大模型错误定位实证研究/LLM_In_Novice_Program_FL/LocalTest/main_code/prompts"

    prompt_location = os.path.join(base_location,"prompt1-all.txt")
    with open(prompt_location, 'r', encoding='utf-8') as file:
        # 读取文件内容并保存到字符串中
        prompt = file.read()
    run_Codeflaws(prompt, "prompt1-all-test", experiment_model, 50)
    # run_Condefects(prompt, "prompt1-all2", experiment_model, 50)
